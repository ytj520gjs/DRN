...
class Net_TransformerEncoder(torch.nn.Module):
    def __init__(self, out_features):
        super(Net_TransformerEncoder, self).__init__()
        self.conv1 = XConv(0, 48, dim=3, kernel_size=8, hidden_channels=32)
        self.conv2 = XConv(
            48, 96, dim=3, kernel_size=12, hidden_channels=64, dilation=2)
        self.conv3 = XConv(
        ...
        encoder_layer = torch.nn.TransformerEncoderLayer(d_model=128, nhead=8) # defualt: d_model=512
        self.transformer_encoder = torch.nn.TransformerEncoder(encoder_layer, num_layers=2)
        self.avgpool = torch.nn.AdaptiveAvgPool1d(1)
        self.avgpool_128 = torch.nn.AdaptiveAvgPool1d(128)
        self.maxpool = torch.nn.MaxPool1d(2, stride=2)
        # self.lin1 = Lin(384, 256)
        self.lin1 = Lin(384, out_features)
        self.lin2 = Lin(256, 128)
    ...
    def forward(self, pos, batch):
        ...
        x = F.relu(self.conv3(x, pos, batch))
        x = F.relu(self.conv4(x, pos, batch))
        # print('before_global_x:', x.shape)
        # x = global_mean_pool(x, batch)
        x = x.reshape(batch_size, 384, -1)
        x = self.avgpool_128(x)
        # print('before_global_x:', x.shape)
        # x, hc = self.rnn(x.reshape(batch_size, 384, -1))
        # print('before_tranfomer_x:', x.shape)

        x = self.transformer_encoder(x)
        # print('tranfomer_x:', x.shape)
...
model = Net_TransformerEncoder(out_features)
# model = Net_Lstm(out_features)
